function [GCAMP] = baselineNormalized_PeriEventTraces_Probability(GCAMP)
...
%% Get the real rewarded trials
for i = 1:length(GCAMP.RE_ON_timestamps)
    GCAMP.Met_Reward(i,:) = nearestpoint(GCAMP.RE_ON_timestamps(i), GCAMP.LP_ON_timestamps,'previous');
end
% Remove any NaN indices (no LP found)
if ~isempty(find(isnan(GCAMP.Met_Reward)))
    idx_remove = find(isnan(GCAMP.Met_Reward));
    GCAMP.Met_Reward(idx_remove) = [];
end
rewarded_idx = [];
for x = 1:length(GCAMP.Met_Reward)
    rewarded_idx = [rewarded_idx ; find(GCAMP.Met_Reward(x) == GCAMP.Criteria_met)];
end

GCAMP.Met_No_Reward = GCAMP.Criteria_met;
GCAMP.Met_No_Reward(rewarded_idx) = [];
...
First_HE_After_Met_noRE_timestamps = GCAMP.HE_ON_timestamps(GCAMP.First_HE_After_Met_noRE);
...
delta_F_First_HE_After_Met_noRE = zeros(length(GCAMP.base_time_end:1/GCAMP.SR:GCAMP.time_end),...
     length(First_HE_After_Met_noRE_timestamps))';
delta_F_First_HE_After_Met_noRE_Baselines_Idx = zeros(1,...
     length(First_HE_After_Met_noRE_timestamps))'; 
...
GCAMP.baseline_norm_LP_ON_Met_Reward = GCAMP.baseline_norm_LP_ON(GCAMP.Met_Reward, :);
GCAMP.baseline_norm_LP_ON_Met_No_Reward = GCAMP.baseline_norm_LP_ON(GCAMP.Met_No_Reward, :);
...
GCAMP.baseline_norm_LP_OFF_Met_Reward = GCAMP.baseline_norm_LP_OFF(GCAMP.Met_Reward,:);
GCAMP.baseline_norm_LP_OFF_Met_No_Reward = GCAMP.baseline_norm_LP_OFF(GCAMP.Met_No_Reward,:);
...
%% For each Successful Lever Press that was not rewarded, extract peri-event (first head entry after press) GCAMP traces
    
for head_entry = 1:length(First_HE_After_Met_noRE_timestamps)
    % Find indices of first head entry after reward timestamps closes to GCAMP trace
    % timestamps
    Closest_HE_ON_idx = nearestpoint(First_HE_After_Met_noRE_timestamps(head_entry),GCAMP.gcampdata_timestamps);
    % Find index of the lever press that led to this head entry
    Closest_LP_before_HE_ON_idx = nearestpoint(First_HE_After_Met_noRE_timestamps(head_entry),GCAMP.LP_ON_timestamps,'previous');
    LP_before_HE_ON_timestamp = GCAMP.LP_ON_timestamps(Closest_LP_before_HE_ON_idx);
    LP_Idx_before_HE_ON_timestamp = nearestpoint(LP_before_HE_ON_timestamp,GCAMP.gcampdata_timestamps);
    % Extract peri-event traces for first head entry after reward
    data_window_HE_ON = GCAMP.gcampdata(Closest_HE_ON_idx + GCAMP.base_time_end * GCAMP.SR : Closest_HE_ON_idx + GCAMP.time_end * GCAMP.SR)';
    delta_F_First_HE_After_Met_noRE(head_entry,:) = data_window_HE_ON;
    % Extract pre-first head entry after reward baseline traces
    %Pre_First_HE_After_RE_baseline = GCAMP.gcampdata(LP_Idx_before_HE_ON_timestamp + GCAMP.base_time_start * GCAMP.SR : LP_Idx_before_HE_ON_timestamp + GCAMP.base_time_end * GCAMP.SR);
    delta_F_First_HE_After_Met_noRE_Baselines_Idx(head_entry,:) = Closest_LP_before_HE_ON_idx;

end
% Normalize peri-event GCAMP traces to pre-lever press onset baselines
GCAMP.baseline_norm_First_HE_After_Met_noRE = (delta_F_First_HE_After_Met_noRE - delta_F_Pre_LP_ON_Baselines_Mean(delta_F_First_HE_After_Met_noRE_Baselines_Idx)) ./ delta_F_Pre_LP_ON_Baselines_STD(delta_F_First_HE_After_Met_noRE_Baselines_Idx);
...
%% For each Lever Press, linearly interpolate the continuous activity during each ongoing lever press
    ...
    for lever_press = 1:length(GCAMP.HoldDown_times) % for each lever press
        ...
        % timestamps
        Closest_LP_ON_idx = nearestpoint(GCAMP.LP_ON_timestamps(lever_press),GCAMP.gcampdata_timestamps, 'next');
        Closest_LP_OFF_idx = nearestpoint(GCAMP.LP_OFF_timestamps(lever_press),GCAMP.gcampdata_timestamps, 'previous');
        ...
        if  total_samples_in_duration(lever_press,:) >= 2 % Interpolation requires at least two sample points of duration activity]
            duration_activity = baseline_z_score_LP_Duration{lever_press}; % variable vector of F (determined by duration)
...
% Normalize peri-event GCAMP traces to pre-lever press onset baselines
GCAMP.baseline_norm_Duration = interp_data;
GCAMP.baseline_norm_Duration_Met_Reward = GCAMP.baseline_norm_Duration(GCAMP.Met_Reward, :);
GCAMP.baseline_norm_Duration_Met_No_Reward = GCAMP.baseline_norm_Duration(GCAMP.Met_No_Reward, :);
